...
**Disposition: three lifecycle boundaries approved; remaining #20 semantics deferred.**
...
#20 must still decide transition details, expiration/deadline behavior, and any later
availability or provider semantics. Do not build calendar integration before the
complete #20 contract is accepted and separately authorized.
...
| [Calendar bake-off](bakeoff-calendar-scheduling.md) | Howbout remains the closest “undated idea to dated plan” model. Apple/Google remain primitives, not an idea system. | Preserve one stable Plan with append-only revisions, model recurring offers as templates plus discrete windows, and use a user-confirmed V1 calendar handoff; finish transition-detail, deadline, and privacy decisions before implementation. |
...
| Undated candidate -> confirmed Plan transition | **Partially approved; remaining #20 contract deferred** | A confirmed Plan has one stable identity and append-only reschedule/cancellation history; recurring offers use templates plus discrete windows; deadlines and transition details remain unresolved. |
...
1. Which append-only transition details must every Plan revision retain for a
   reschedule or cancellation, including actor, reason, prior values, and calendar
   export provenance?
2. When do specials and reservation deadlines expire, and how do expiration states
