...
| 20 | 2026-07-19 | The user approved the recommended sequence; PR #28 was clean, mergeable, and reported a successful CodeRabbit check, then merged to `origin/main` as `35533dc`. Issue #24 is now the implementation frontier, but its legacy-client compatibility criterion conflicts with the user-approved greenfield debt gate. Research: reused — `docs/research/api-first-backend-services.md` already establishes the provider-neutral `/v1` contract, D1 persistence, deterministic explainable ranking, and fixed-household architecture used by this slice. Last30Days: not needed — #24 executes a fixed product policy and locally testable historical replay contract; recent practitioner recommendations cannot change its accepted constants or public seams. | Start `codex/versioned-preference-policy-24` from merged `origin/main`; replace the stale compatibility criterion with one canonical preference/history path and behavioral stability only for current capture, notes, and metadata. Use the approved public seams—domain ranking/replay, `/v1` Worker contract, generated transport, responsive UI, and Playwright—for red-green tracer bullets. Keep #25 blocked, and keep deployment, paid services, production credentials/data, native distribution, and Shortcut rebuild/signing outside authority. |
| 21 | 2026-07-19 | The issue #24 implementation intentionally changed controller-owned inputs after its initial lease, then the lease expired before checkpoint. The next wake correctly invalidated revision-bound evidence and returned `noop` with `dirty-tree-conflict`; the user explicitly authorized repairing the conflict and continuing autonomously. The complete dirty inventory contains the issue #24 domain, API, generated client, web, tests, documentation, and append-only controller records, with no protected Shortcut changes. | Preserve and verify the implementation as one greenfield slice, commit it logically, then reconcile the controller against the new clean HEAD and obtain a fresh lease. Do not bypass the fence, discard dirty work, preserve compatibility code, or reuse pre-commit evidence for final completion. Publication remains a separate authority boundary. |
| 22 | 2026-07-19 | The first fresh semantic gate rejected `f097a00`: although one preference projection fed both rank and visible preferences, `detail()` read the honeymoon-period row separately before preferences, notes, and captures, permitting a concurrent rank-boost or preference write to produce a mixed-time response. The controller recorded this as the first bounded repair cycle; this entry corrects the durable log immediately after that checkpoint rather than concealing the ordering defect. | Replace the split detail reads with one transactional D1 batch and add a regression test that observes exactly one detail batch. Re-run the aggregate, commit the correction, reconcile revision-bound evidence, and obtain fresh independent ACCEPT and PASS gates. The rejected verdict is retained as evidence and cannot authorize completion. |
