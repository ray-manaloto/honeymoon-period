...
metadata:
    github-path: plugins/build-web-apps/skills/frontend-testing-debugging
    github-pinned: 11c74d6ba24d3a6d48f54a194cd00ef3beea18f9
    github-ref: 11c74d6ba24d3a6d48f54a194cd00ef3beea18f9
    github-repo: https://github.com/openai/plugins
    github-tree-sha: e31dfad87123a11167c1a610d7bd5560cb787ef7
...
2. Name the session with `browser.nameSession("...")` using the selected binding.
3. Acquire a tab with `browser.tabs.selected()` or `browser.tabs.new()`.
...
5. **Screenshot evidence**: use the screenshot method documented by the selected
   Browser runtime. For the current in-app Browser this is
   `await nodeRepl.emitImage(await tab.screenshot({ fullPage: false }))`, not a
   guessed Playwright screenshot method.
...
Responsive layouts may keep hidden desktop and mobile controls in the DOM,
which can make otherwise sensible role or `href` locators resolve more than
once. Re-snapshot, scope to the visible container, and confirm uniqueness. When
navigation itself is not the behavior under test, direct navigation to a known
local route is preferable to selecting an arbitrary duplicate.
